...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        path1 = path + '/A'
        path2 = path + '/B'
        p1 = str(Path(path1).resolve())  # os-agnostic absolute path
        p2 = str(Path(path2).resolve())
        if '*' in p1:
            files1 = sorted(glob.glob(p1, recursive=True))  # glob
            files2 = sorted(glob.glob(p2, recursive=True))
        elif os.path.isdir(p1):
            files1 = sorted(glob.glob(os.path.join(p1, '*.*')))  # dir
            files2 = sorted(glob.glob(os.path.join(p2, '*.*')))
        ...
        images1 = [x for x in files1 if x.split('.')[-1].lower() in IMG_FORMATS]
        videos1 = [x for x in files1 if x.split('.')[-1].lower() in VID_FORMATS]
        images2 = [x for x in files2 if x.split('.')[-1].lower() in IMG_FORMATS]
        videos2 = [x for x in files2 if x.split('.')[-1].lower() in VID_FORMATS]
        ni, nv = len(images1), len(videos1)
        ...
        self.auto = auto
        if any(videos1):
            self.new_video(videos1[0])  # new video
        else:
            self.cap = None
        assert self.nf > 0, f'No images or videos found in {p1}. ' \
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # Read image
            self.count += 1
            img10 = cv2.imread(path1)  # BGR
            img20 = cv2.imread(path2)
            assert img10 is not None, f'Image Not Found {path1}'
            s = f'image {self.count}/{self.nf} {path1}: '

        # Padded resize
        im = letterbox(img10, img20, self.img_size, stride=self.stride, auto=self.auto)
        img1, img2 = im[0], im[1]
        ...
        return path1, img1, img10, path2, img2, img20, self.cap, s  # img为经过处理的，img0为原始图片
...
def img2label_paths(img_paths):
    ...
    sc = f'{os.sep}A{os.sep}'
    return ['/'.join(sb.join(x.rsplit(sa, 1)).rsplit(sc, 1)).rsplit('.', 1)[0] + '.txt' for x in img_paths]
    # return [sb.join(x.rsplit(sa, 1)).rsplit('.', 1)[0] + '.txt' for x in img_paths]
def img2label_paths2(img_paths):
    # Define label paths as a function of image paths
    sa, sb = f'{os.sep}images{os.sep}', f'{os.sep}labels{os.sep}'  # /images/, /labels/ substrings
    sc = f'{os.sep}B{os.sep}'
    return ['/'.join(sb.join(x.rsplit(sa, 1)).rsplit(sc, 1)).rsplit('.', 1)[0] + '.txt' for x in img_paths]
...
def get_cache(path, mode,prefix):
    ...
    if mode == 1:
        ...
        cache_path = (p if p.is_file() else Path(str(Path(label_files[0]).parent)+'_A')).with_suffix('.cache')
    elif mode == 2:
        label_files = img2label_paths2(im_files)  # 获取labels
        cache_path = (p if p.is_file() else Path(str(Path(label_files[0]).parent)+'_B')).with_suffix('.cache')
    try:
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        if cache_images:
            ...
            fcn = self.cache_images_to_disk if cache_images == 'disk' else self.load_image1
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < hyp['mixup']:
                # im1_random, im2_random, imlabel_random = self.load_mosaic(random.randint(0, self.n - 1))
                im_random = self.load_mosaic(random.randint(0, self.n - 1))
                img, labels = mixup(img, labels, im_random[0], im_random[2])
                img2, _ = mixup(img2, labels, im_random[1], im_random[2])

        else:
            # Load image
            img, (h0, w0), (h, w) = self.load_image1(index)
            img2, (h02, w02), (h2, w2) = self.load_image2(index)

            # Letterbox
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img, img2, ratio, pad = letterbox(img, img2, shape, auto=False, scaleup=self.augment)

            # img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)
            shapes = (h0, w0), ((h / h0, w / w0), pad)  # for COCO mAP rescaling
            labels = self.labels[index].copy()
            # img2, ratio2, pad2 = letterbox(img2, shape, auto=False, scaleup=self.augment)
            shapes2 = (h02, w02), ((h2 / h02, w2 / w02), pad)  # for COCO mAP rescaling
    ...
    def load_image2(self, i):
        ...
        if im is None:  # not cached in RAM
            ...
            return self.ims2[i], self.im_hw0[i], self.im_hw[i]
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            # Load image
            img, _, (h, w) = self.load_image1(index)
            img2, _, (h2, w2) = self.load_image2(index)

            # place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img4_2 = np.full((s * 2, s * 2, img2.shape[2]), 114, dtype=np.uint8)
            ...
            img4_2[y1a:y2a, x1a:x2a] = img2[y1b:y2b, x1b:x2b]
            padw = x1a - x1b
            padh = y1a - y1b
        ...
        # img4_2, labels4, segments4 = copy_paste(img4_2, labels4, segments4, p=self.hyp['copy_paste'])
        # img4, img4_2, labels4, segments4 = copy_paste(img4, img4_2, labels4, segments4, p=self.hyp['copy_paste'])
        # 随机变换
        img4, img4_2, labels4 = random_perspective(img4, img4_2,
                                           labels4,
                                           segments4,
    ...
    def collate_fn4(batch):
        ...
        n = len(shapes) // 4
        im4,im42, label4, path4,path42, shapes4,shapes42 = [],[], [], path[:n],path2[:n],shapes[:n],shapes2[:n]
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                ...
                im2 = F.interpolate(img2[i].unsqueeze(0).float(), scale_factor=2.0, mode='bilinear',
                                   align_corners=False)[0].type(img2[i].type())
                lb = label[i]
            else:
                im = torch.cat((torch.cat((img[i], img[i + 1]), 1), torch.cat((img[i + 2], img[i + 3]), 1)), 2)
                im = torch.cat((torch.cat((img2[i], img2[i + 1]), 1), torch.cat((img2[i + 2], img2[i + 3]), 1)), 2)
