...
LOGGER = logging.getLogger("yolov5")  # define globally (used in train_load.py, val.py, detect.py, etc.)
...
def check_dataset(data, autodownload=True):
    ...
    # Checks
    for k in 'train', 'val', 'train2', 'val2', 'nc':
    ...
    for k in 'train', 'val', 'test', 'train2', 'val2', 'test2':
        if data.get(k):  # prepend path
            data[k] = str(path / data[k]) if isinstance(data[k], str) else [str(path / x) for x in data[k]]

    # Parse yaml
    train, val, test, train2, val2, test2, s = (data.get(x) for x in ('train', 'val', 'test', 'train2', 'val2', 'test2', 'download'))
    if val:
...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    # Increment file or directory path, i.e. runs/exp_our --> runs/exp_our{sep}2, runs/exp_our{sep}3, ... etc.
