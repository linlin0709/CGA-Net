...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Process 0
    if RANK in {-1, 0}:
        val_loader = create_dataloader(val_path, val_path2,
                                       imgsz,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, imgs2, targets, paths, paths2, _, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # 随机交换
            bs_s = imgs.size(0)
            max_num_swaps = bs_s // 2
            num_swaps = torch.randint(0, max_num_swaps+1, (1,)).item()
            indices_s = torch.randperm(bs_s)[:num_swaps]
            x_imgs = imgs.clone()
            x_imgs2 = imgs2.clone()
            for s in indices_s:
               x_imgs[s], x_imgs2[s] = imgs2[s], imgs[s]
            imgs = x_imgs
            imgs2 = x_imgs2
            ...
            if opt.multi_scale:
                ...
                if sf != 1 and sf2 != 1:
                    ...
                    ns2 = [math.ceil(x * sf / gs) * gs for x in imgs2.shape[2:]]  # new shape (stretched to gs-multiple)
            ...
            with torch.cuda.amp.autocast(amp):
                # imgs11 = torch.cat((torch.abs(imgs - imgs2), imgs), dim=1)
                # imgs21 = torch.cat((torch.abs(imgs - imgs2), imgs2), dim=1)
                # imgs = imgs11
                # imgs2 = imgs21
            ...
            if RANK in {-1, 0}:
                ...
                callbacks.run('on_train_batch_end', ni, model, imgs,imgs2,targets, paths,paths2, plots)
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = val.run(
                        ...
                        # iou_thres=0.4,  # best pycocotools results at 0.65
                        single_cls=single_cls,
                        dataloader=val_loader,
                        save_dir=save_dir,
                        save_json=is_coco,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default=ROOT /'yolov5l-dual-difference_Anchor_ExFeature_F_GATtry_D_med.yaml', help='model.yaml path')
